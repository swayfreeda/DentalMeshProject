...
namespace SW
{
   ...
   void  Mesh::draw(int flag){
       ...
       case 0: // draw vertex
           ...
           glBegin(GL_POINTS);
           // all the vertices
           for(VertexCIter vit = vertices.begin();
               vit!= vertices.end(); vit++){
               glVertex3f(vit->position.x, vit->position.y, vit->position.z );
           }
       ...
       case 1: // draw edges
           ...
           // all the vertices
           for(EdgeCIter eit = edges.begin();
               eit!= edges.end(); eit++){
               if(eit->he->onBoundary) continue;

               glBegin(GL_LINES);
               HalfEdgeCIter he = eit->he;
               do{
       ...
       case 2:
           ...
           glColor3f(1.0, 1.0, 1.0);
           for (FaceCIter fit = faces.begin(); fit != faces.end(); fit ++ )
             {
                 if( fit->isBoundary() ) continue;
                 glBegin( GL_POLYGON );

                 Vector N = fit->normal();
                 glNormal3dv( &N[0] );
                 HalfEdgeCIter he = fit->he;
                 do
                 {
                     glVertex3dv( &he->vertex->position[0] );
                     he = he->next;
